...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            // 对管理员的下拉列表框进行远程加载
            $('#cc').combobox({
                // 将请求发送给accountAction中的query方法处理，这里需要将处理好的数据返回到这边来显示，所以后台需要将数据打包成json格式发过来
                url: 'account_query.action',
                valueField: 'id',
                textField: 'login',// 下拉列表中显示的管理员登录名
                panelHeight: 'auto',// 自适应高度
                panelWidth: 120,// 下拉列表是两个组件组成的
                width: 120,// 要同时设置两个宽度才行
                editable: false// 下拉框不允许编辑
            });
            // 窗体弹出默认是禁用验证的，因为刚弹出的窗口，用户还没填就显示的话不美观
            $('#ff').form('disableValidation');
            // 注册button事件。即当用户点击“添加”时要做的事
            $('#btn').click(function () {
                // 开启验证
